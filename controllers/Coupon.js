    ...
    .then((coupon) => {
      if(coupon == null){
        ...
        .then((result) => {
          ...
          res
            .status(201)
            .json({ message: "Coupon created successfully", newCoupon: result });
        })
        .catch((err) => {
          console.log(err);
        });
...
exports.getCoupon = (req, res) => {
  Coupon.find({status: { $ne: "DELETED" },usuage: { $ne: "0" }, $or: [ { type:"public"}, {students :{$elemMatch:{ id : "612de254c11c8748a47179d8"}}} ] })
    .then((coupon) => {
...
exports.verifyCoupon = (req, res) => {
  var doc = req.body;
  Coupon.findOne({$and: [{ couponcode: doc.couponCode }, {usuage: { $ne: 0} }]} )
  .then((coupon) => {
    if(coupon.type == "private"){
      Coupon.find({$and:[{ couponcode: doc.couponCode },{students :{$elemMatch:{$and:[{id : doc.userId},{status: "NA"}] }}}]}  )
      .then((result) => {
        if(result.length != 0){
          ...
          res.status(200).json({message: "Success", couponData: result, finalAmount: finalAmount });
        }else {
      ...
      var finalAmount = parseInt(doc.cartAmount) * (discount/100);
      finalAmount = doc.cartAmount - finalAmount;
      res.status(200).json({message: "Success", couponData: coupon, finalAmount: finalAmount });
    }
  })
  .catch((err) => {
    console.log(err);
...
exports.deleteCoupon = (req, res) => {
  ...
  { $set: { "status" : "DELETED" } }).then((coupon) => {
